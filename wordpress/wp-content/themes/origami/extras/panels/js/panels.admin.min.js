...
num=Math.round(num);num=Math.max(1,num);num=Math.min(10,num);var gridContainer=window.panels.createGrid(num);gridContainer.hide().slideDown();$('#grid-add-dialog').dialog('close');};$('#grid-add-dialog').show().dialog({dialogClass:'panels-admin-dialog',autoOpen:false,modal:true,title:$('#grid-add-dialog').attr('data-title'),open:function(){$(this).find('input').val(2).select();},buttons:gridAddDialogButtons}).keypress(function(e){if(e.keyCode==$.ui.keyCode.ENTER){$(this).closest('.ui-dialog').find('.ui-button:eq(0)').click();}});;$('#so-panels-panels .handlediv').click(function(){setTimeout(function(){$(window).resize();},150);})
$('#panels-dialog').show().dialog({dialogClass:'panels-admin-dialog',autoOpen:false,resizable:false,draggable:false,modal:true,title:$('#panels-dialog').attr('data-title'),minWidth:960,close:function(){$('#panels-container .panel.new-panel').hide().slideDown(1000).removeClass('new-panel');}}).find('.panel-type').disableSelection();$('#panels .panels-add').button({icons:{primary:'ui-icon-add'},text:false}).click(function(){$('#panels-text-filter-input').val('').keyup();$('#panels-dialog').dialog('open');return false;});$('#panels .grid-add').button({icons:{primary:'ui-icon-columns'},text:false}).click(function(){$('#grid-add-dialog').dialog('open');return false;});$('#panels-text-filter-input').keyup(function(e){if(e.keyCode==13){var p=$('#panels-dialog .panel-type-list .panel-type:visible');if(p.length==1)p.click();return;}
